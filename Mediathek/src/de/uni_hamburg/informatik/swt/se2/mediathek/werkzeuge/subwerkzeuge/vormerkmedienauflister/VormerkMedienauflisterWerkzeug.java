...
public class VormerkMedienauflisterWerkzeug extends ObservableSubWerkzeug
{
    ...
    private void setzeAnzuzeigendeMedien()
    {
        ...
        for (Medium medium : medienListe)
        {
        	...
        	anzahlVormerker = _verleihService.getVormerkkarteFuer(medium).getAnzahlVormerker();
        	
        	for (int i = 0; i < anzahlVormerker; i++)
        	{
        		vormerkerListe[i] = _verleihService.getVormerkkarteFuer(medium).getVormerker(i);
        		System.out.println(vormerkerListe[i]);
        	}
